  ...
  bootstrap (context) {
    return new Promise((resolve, reject) => {
      Service.get('/api/bootstrap').then((response) => {
        ...
        context.commit('preferences/SET_MOMENT_DATE_FORMAT', response.data.moment_date_format)
        context.commit('preferences/SET_LANGUAGE_FORMAT', response.data.default_language)
